...
const defaultMaterials = `const defaultMaterials = ['PLA', 'PETG', 'ABS', 'TPU', 'ASA'];`;
const defaultBrands = `const defaultBrands = ['Matte', 'Basic', 'Silk', 'Pro', 'Plus'];`;
const initSettings = `function initSettings() {
            ...
            let materials = JSON.parse(localStorage.getItem('materials')) || ${JSON.stringify(['PLA', 'PETG', 'ABS', 'TPU', 'ASA'])};
            let brands = JSON.parse(localStorage.getItem('brands')) || ${JSON.stringify(['Matte', 'Basic', 'Silk', 'Pro', 'Plus'])};
...
console.log('filament.html updated successfully with settings button');
process.exit(0);